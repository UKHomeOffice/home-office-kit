...
function getHomeOfficeKitConfig() {
  ...
  if (prototypeConfig.pluginConfig) {
    homeOfficeKitConfig = prototypeConfig.pluginConfig['home-office-kit']

    // If logging the prototype kit data, then by default ignore /plugin-assets/, /public/ and /manage-prorotype/page-found requests
    if (homeOfficeKitConfig.logData && homeOfficeKitConfig.logData.ignoreUrlsStartingWith===undefined) {
      homeOfficeKitConfig.logData.ignoreUrlsStartingWith = ["/plugin-assets/", "/public/", "/manage-prototype/page-found"]
    }
  } else {
    homeOfficeKitConfig = {}
  }
